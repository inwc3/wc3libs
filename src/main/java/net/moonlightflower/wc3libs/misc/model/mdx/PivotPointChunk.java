...
public class PivotPointChunk extends Chunk {
    ...
    private List<PivotPoint> _pivotPoints = new ArrayList<>();

    public List<PivotPoint> getPivotPoints() {
        return new ArrayList<>(_pivotPoints);
    }

    public void addPivotPoint(@Nonnull PivotPoint val) {
        if (!_pivotPoints.contains(val)) {
            _pivotPoints.add(val);
        }
    }
    ...
    private void read_0x0(@Nonnull Wc3BinInputStream stream) throws BinInputStream.StreamException {
        ...
        while (stream.getPos() < endPos) {
            addPivotPoint(new PivotPoint(stream));
        }
    }
    ...
    public PivotPointChunk(@Nonnull Wc3BinInputStream stream, @Nonnull MDX.EncodingFormat format) throws BinInputStream.StreamException {
        this();

        read(stream, format);
    }

    public PivotPointChunk() {
